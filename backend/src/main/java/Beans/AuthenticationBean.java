...
public class AuthenticationBean {
    ...
    public Tokens login(LoginRequest request) throws LoginException {
        ...
        if (!CryptUtility.checkPassword(request.getPassword(), user.getPassword()))
            throw LoginException.wrongPassword("Wrong password");

        String accessToken = JWTUtility.tokenForUser(user);
    ...
    public void registration(RegistrationRequest request) throws RegistrationException {
        AuthenticationBean.validate(request);
        ...
        String hashedPassword = CryptUtility.hashPassword(request.getPassword1());

...
import DataBaseModel.ResultDB;
import DataBaseModel.UserDB;
...
public class ResultsBean {
    ...
    public CheckResponse check(Coordinates coordinates, UserPrincipal userPrincipal) {
        try {
            ...
            ResultDB result = ResultDB.builder().x(coordinates.getX()).y(coordinates.getY()).r(coordinates.getR()).success(success).timestamp(LocalDateTime.now()).owner(user).build();
    ...
    public List<Result> getResults(UserPrincipal userPrincipal) {
        try {
            ...
            return user.getResults().stream().sorted(Comparator.comparing(ResultDB::getTimestamp)).map(ResultsBean::transformToResult).collect(Collectors.toList());
    ...
    private static Result transformToResult(ResultDB db) {
        return new Result(db.getId(), String.valueOf(db.getX()), String.valueOf(db.getY()), String.valueOf(db.getR()), db.getTimestamp().toString(), db.getSuccess());
    }
}

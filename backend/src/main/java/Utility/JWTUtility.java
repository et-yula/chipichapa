...
public class JWTUtility {
    ...
    public static String tokenForUser(UserDB user) {
        ...
        return Jwts.builder().setIssuedAt(now).setExpiration(expiration).claim(USER_ID_CLAIM, user.getId().toString()).claim(USERNAME_CLAIM, user.getUsername()).signWith(SignatureAlgorithm.HS512, TextCodec.BASE64.decode(SECRET_KEY)).compact();
    }
